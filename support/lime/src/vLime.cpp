...
Device::Device(vk::PhysicalDevice pd, Capabilities& capabilities)
    : pDevice(pd)
    , preferredDepthFormat(getPreferredDepthFormat())
{
...
void PrintAllPhysicalDevices(vk::Instance i)
{
    std::vector<vk::PhysicalDevice> pDevices;
    auto const pdGroups { check(i.enumeratePhysicalDeviceGroups()) };

    unsigned counter = 0;
    for (auto const& pdg : pdGroups) {
        log::info("Device group " + std::to_string(counter++) + ":");
        for (u32 j = 0; j < pdg.physicalDeviceCount; j++) {
            pDevices.emplace_back(pdg.physicalDevices[j]);
            auto properties { pdg.physicalDevices[j].getProperties() };
            std::string pdName = properties.deviceName;
            log::info(" -> " + pdName);
        }
    }
}
