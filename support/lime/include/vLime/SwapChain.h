...
public:
    ...
    vk::Extent2D Resize(u32 x, u32 y)
    {
        createSwapChain(x, y);
        return extent;
    }
    ...
    void scheduleRgTaskLayoutTransition(rg::Graph& rg, vk::ImageLayout finalLayout)
    {
        ...
        auto const taskId { rg.AddTask<rg::Commands>() };
    ...
    u32 imageIndexToPresentNext { INVALID_ID };
...
private:
    ...
    void createSwapChain(u32 x = 0, u32 y = 0)
    {
        ...
        extent = chooseSwapExtent(swapChainSupport.capabilities, x, y);
    ...
    static vk::Extent2D chooseSwapExtent(vk::SurfaceCapabilitiesKHR const& capabilities, u32 x, u32 y)
    {
        ...
        if (x > 0 && y > 0)
            return {
                .width = std::max(capabilities.minImageExtent.width, std::min(capabilities.maxImageExtent.width, x)),
                .height = std::max(capabilities.minImageExtent.height, std::min(capabilities.maxImageExtent.height, y)),
            };
