...
# command = ['glslangValidator', '-gVS', '--target-env', 'vulkan1.3', '--target-env', 'spirv1.6', '', '-o', '']
...
def find_glsl_dep(src, depSet):
    ...
    while True:
        ...
        if (l.startswith('#include "')):
            dep = Path(src.parent / l.split('"')[1])
            if dep.exists() and dep not in depSet:
                depSet.add(dep)
